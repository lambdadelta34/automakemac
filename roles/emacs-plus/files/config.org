...
#+BEGIN_SRC emacs-lisp
(use-package eshell
  :hook ((eshell-mode . (lambda () (setq-local company-backends '(company-files company-capf)))))
  :ensure nil
  :preface
  (defun my/eshell ()
    (interactive)
...
(use-package haskell-mode
  ...
  (defun my/autocomplete ()
    (setq-local company-backends '(company-ghc company-dabbrev-code)))
  :init
...
(use-package ruby-mode
  ...
  (defun my/rubocop-fix-file ()
    (interactive)
    (message "rubocop fixing the" (buffer-file-name))
    (with-temp-buffer
      (shell-command (format "%s %s" flycheck-ruby-rubocop-executable
                            (concat "-a " (buffer-file-name)))
                    t)))
  ...
  (:states 'normal
    :prefix leader
    "rfx" 'my/rubocop-fix-file-and-revert)
