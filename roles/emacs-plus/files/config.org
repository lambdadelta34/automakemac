...
(use-package ivy
  :defer t
  :diminish (ivy-mode . "")
  ;; define here function which opens provided file
  ;; :preface
  ...
  :config
  ;; sets actions in minibuffer. gets string as arg. and grep result can lead to file.
  ;; or i may try to redirect whole search result to ranger
  ;; (ivy-set-actions
  ;; t
...
* TODOs
 - Search with preview
   - [[https://github.com/abo-abo/swiper/wiki/ivy-display-function][Start from here]].
* Problems to solve
** TODO .dir-locals.el with ranger
It just freezes. Fix that.
** TODO emacsclient from mac os launchpad
